...
func NewRootCmd() (*cobra.Command, boli.EncodingConfig) {
	var (
		config  = boli.MakeEncodingConfig()
		context = client.Context{}.
			...
			WithHomeDir(boli.DefaultNodeHome)
	)
...
func initRootCmd(rootCmd *cobra.Command, encoding boli.EncodingConfig) {
	rootCmd.AddCommand(
		genutilcli.InitCmd(boli.ModuleBasics, boli.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, boli.DefaultNodeHome),
		genutilcli.GenTxCmd(boli.ModuleBasics, encoding.TxConfig, banktypes.GenesisBalancesIterator{}, boli.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(boli.ModuleBasics),
		AddGenesisAccountCmd(boli.DefaultNodeHome),
		AddGenesisWasmMsgCmd(boli.DefaultNodeHome),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(boli.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)

	server.AddCommands(rootCmd, boli.DefaultNodeHome, appCreatorFunc, appExportFunc, addModuleInitFlags)
	rootCmd.AddCommand(
		rpc.StatusCommand(),
		queryCommand(),
		txCommand(),
		keys.Commands(boli.DefaultNodeHome),
	)
}
...
func queryCommand() *cobra.Command {
	...
	boli.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	boli.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func appCreatorFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, options servertypes.AppOptions) servertypes.Application {
	...
	return boli.New(
		...
		boli.MakeEncodingConfig(),
		options,
		boli.GetWasmEnabledProposals(),
		wasmOpts,
...
func appExportFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, height int64,
	...
	var app *boli.App
	if height != -1 {
		app = boli.New(logger, db, tracer, false, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, boli.GetWasmEnabledProposals(), emptyWasmOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = boli.New(logger, db, tracer, true, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, boli.GetWasmEnabledProposals(), emptyWasmOpts)
	}
